...
$is_multi = isset($_GET['multi']) && $_GET['multi'] === '1';
$booking_count = isset($_GET['count']) ? (int)$_GET['count'] : 1;
...
// If this is a multi-booking, fetch all related bookings
$bookings = [];
if ($is_multi) {
    $query = "SELECT b.*, sv.ServiceType, sch.Sched_Date, sch.Time_Start, sch.Time_End,
                     i.Name AS InstructorName
              FROM bookings b
              JOIN services sv ON b.ServiceID = sv.ServiceID
              JOIN schedules sch ON b.ScheduleID = sch.ScheduleID
              LEFT JOIN instructors i ON b.InstructorID = i.InstructorID
              WHERE b.ClientID = ? AND b.BookingID >= ?
              ORDER BY sch.Sched_Date, sch.Time_Start
              LIMIT ?";

    $stmt = mysqli_prepare($conn, $query);
    $first_booking_id = $booking_id;
    mysqli_stmt_bind_param($stmt, "iii", $_SESSION['user_id'], $first_booking_id, $booking_count);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
        ...
        <div class="fullwidth-block" style="padding: 60px 20px;">
            <div class="confirmation-container">
                ...
                <div class="booking-summary">
                    ...
                    <?php if (!$is_multi && !empty($booking_addons)): ?>
                        <div class="time-slots">
                            <h4><i class="fas fa-guitar"></i> Instruments Added</h4>
                            <div class="slots-grid" style="display:block;">
                                <?php foreach ($booking_addons as $addon): ?>
                                    <div class="slot-card" style="display:flex; justify-content:space-between; align-items:center;">
                                        <div class="slot-service" style="color:#fff;">
                                            <i class="fas fa-music"></i>
                                            <span><?php echo htmlspecialchars($addon['Name']); ?></span>
                                        </div>
                                        <div class="slot-price" style="color:#fff;">
                                            <span>× <?php echo (int)$addon['Quantity']; ?></span>
                                            <span style="margin-left:12px;">₱<?php echo number_format($addon['Price'], 2); ?></span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                    ...
                    <div class="payment-content">
                        <div class="payment-text">
                            ...
                            <div class="payment-amount">
                                <span class="amount-label">Total Initial Payment:</span>
                                <span class="amount-value">₱<?php echo number_format($total_initial_amount, 2); ?></span>
                            </div>
                            <?php else: ?>
                            <div class="payment-amount">
                                <span class="amount-label">Total Amount:</span>
                                <span class="amount-value">₱<?php echo number_format($booking['TotalAmount'], 2); ?></span>
                            </div>
                            <?php endif; ?>
                        </div>
