...
// Pull messages from chatbot actions (session)
if (isset($_SESSION['chatbot_success_message'])) {
    $success_message = trim(($success_message ? $success_message . ' ' : '') . $_SESSION['chatbot_success_message']);
    unset($_SESSION['chatbot_success_message']);
}
if (isset($_SESSION['chatbot_error_message'])) {
    $error_message = trim(($error_message ? $error_message . ' ' : '') . $_SESSION['chatbot_error_message']);
...
<body>
    ...
    <div class="main-content">
        <div class="settings-container">
                ...
                <div class="stat-card">
                    <div class="stat-header">
                        ...
                        <div>
                            ...
                            <p class="stat-value"><?php echo "Php" . number_format($totalRevenue, 2); ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Settings Tabs -->
            <style>
                .tabs-container { background: linear-gradient(135deg, var(--netflix-dark-gray) 0%, #1a1a1a 100%); border-radius: 12px; padding: 20px; margin-bottom: 30px; border: 1px solid #333; }
                .tabs { display: flex; gap: 10px; }
                .tab { padding: 12px 24px; background: transparent; border: 1px solid #333; border-radius: 8px; color: var(--netflix-light-gray); cursor: pointer; transition: all 0.2s ease; font-weight: 500; display: flex; align-items: center; gap: 8px; }
                .tab:hover { background: var(--netflix-black); color: var(--netflix-white); }
                .tab.active { background: var(--netflix-black); color: var(--netflix-white); border-color: var(--netflix-red); }
            </style>
            <div class="tabs-container fade-in">
                <div class="tabs">
                    <button class="tab active" onclick="window.changeSettingsTab('general')" id="tab-btn-general">
                        <i class="fas fa-cog"></i>
                        General
                    </button>
                    <button class="tab" onclick="window.changeSettingsTab('chatbot')" id="tab-btn-chatbot">
            ...
            <!-- Settings Forms -->
            <div id="settings-forms" class="settings-grid fade-in" aria-label="Settings Forms">
                <!-- Profile Settings -->
            ...
            <!-- Chatbot Tab Content -->
            <div id="chatbot-tab" class="fade-in" aria-label="Chatbot Settings" style="display: none;">
                <?php define('CHATBOT_EMBED', true); include __DIR__ . '/manage_chatbot.php'; ?>
            </div>
    ...
    <script>
        ...
        // Settings tabs
        window.changeSettingsTab = function(tab) {
            const general = document.getElementById('settings-forms');
            const chatbot = document.getElementById('chatbot-tab');
            const btnGeneral = document.getElementById('tab-btn-general');
            const btnChatbot = document.getElementById('tab-btn-chatbot');
            if (!general || !chatbot) return;

            if (tab === 'chatbot') {
                general.style.display = 'none';
                chatbot.style.display = 'block';
                if (btnGeneral) btnGeneral.classList.remove('active');
                if (btnChatbot) btnChatbot.classList.add('active');
                ...
                if (btnChatbot) btnChatbot.classList.remove('active');
                const url = new URL(window.location);
                url.hash = '';
                window.history.replaceState({}, '', url);
            }
        }
        var changeSettingsTab = window.changeSettingsTab;

        // Initialize tab from hash
        (function() {
            const initialTab = (window.location.hash && window.location.hash.toLowerCase().includes('chatbot')) ? 'chatbot' : 'general';
            window.changeSettingsTab(initialTab);
        })();

        // Read chatbot flash from sessionStorage (embedded redirects)
        (function() {
            try {
                const raw = sessionStorage.getItem('chatbot_flash');
                if (raw) {
                    const data = JSON.parse(raw);
                    sessionStorage.removeItem('chatbot_flash');
                    const container = document.querySelector('.settings-container') || document.body;
                    const alert = document.createElement('div');
                    alert.className = 'alert ' + (data.type === 'error' ? 'alert-danger' : 'alert-success') + ' fade-in';
                    alert.innerHTML = (data.type === 'error' ? '<i class="fas fa-exclamation-triangle"></i>' : '<i class="fas fa-check-circle"></i>') + ' ' + (data.text || '');
                    ...
                    if (typeof window.changeSettingsTab === 'function') {
                        window.changeSettingsTab('chatbot');
                    } else {
                        const url = new URL(window.location);
                        url.hash = 'chatbot';
                        window.history.replaceState({}, '', url);
                ...
                console.warn('Failed to render chatbot flash:', e);
            }
        })();
